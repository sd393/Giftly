...
Forked from outreach-brands-audit/process-bounces.py — same DSN handling,
same no-platform-mirror posture. Stdout is summary-only.
"""
import argparse
import csv
import json
...
def mark_bounced(emails: set[str]) -> int:
    ...
    fieldnames = (
        ...
        else ["name", "brand", "email", "date_sent", "verified", "notes"]
    )
    updated = 0
    for row in rows:
