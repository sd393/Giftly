...
Forked from outreach/process-bounces.py — same DSN handling, no platform
mirror integration. Stdout is summary-only.
"""
import argparse
import csv
import json
...
def mark_bounced(emails: set[str]) -> int:
    ...
    fieldnames = (
        ...
        else ["name", "role", "company", "domain", "category", "email", "date_sent", "verified", "notes"]
    )
    updated = 0
    for row in rows:
...
def main():
    ...
    ap.add_argument("--query", help="Override the Gmail search query")
    ap.add_argument("--no-trash", action="store_true", help="Skip trashing DSNs")
    ...
    print(
        ...
        f"log_updated={updated} trashed={trashed} "
