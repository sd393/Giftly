#!/usr/bin/env python3
"""Send the AI-shopping pitch to a verified-emails-only batch of Shopify DTC merchants.
...
Forked from outreach-retailers/send-batch.py with the v5 template verbatim.
Audience: small-to-mid single-brand Shopify DTC merchants ($1-5M MRR band).
Upstream pipeline (source-publicwww.py -> discover-contacts.py ->
verify-emails.py) is what populates the input CSV. Hunter strict gate
guarantees every email here was status=valid + accept_all=false + score>=80.
...
IN_CSV = Path(_args[0]) if _args else (ROOT / "verified.csv")
...
# v5 template, verbatim from outreach-retailers. {name} and {company}
# substitution via render_body(). Fallbacks: empty name -> "Hi,";
...
def append_log(row: dict) -> None:
    fieldnames = ["name", "brand", "email", "date_sent", "verified", "notes"]
...
def main():
    ...
    for i, r in enumerate(targets, 1):
        ...
        brand_raw = r.get("brand") or r.get("retailer") or r.get("company") or ""
        ok, info, body, external_id = send_one(
            email, dry_run=dry, name=name_raw, company=brand_raw,
        )
        status = "OK " if ok else "FAIL"
        id_tag = f" id={external_id}" if external_id else ""
        label = brand_raw or name_raw or "-"
